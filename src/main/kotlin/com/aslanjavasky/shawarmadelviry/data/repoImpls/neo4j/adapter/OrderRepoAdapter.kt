...
import com.aslanjavasky.shawarmadelviry.data.repoImpls.neo4j.entity.*
...
class OrderRepoAdapter(
    ...
    private val menuItemRepository: MenuItemNeo4jRepository
    ...
    override fun updateOrder(order: IOrder): IOrder {
        val existingOrderEntity = orderRepository.findById(order.id!!.getUUIDFromLong())
    ...
    override fun getOrdersByStatus(orderStatus: OrderStatus): List<IOrder> {
        return orderRepository.findByStatus(orderStatus).map { it.toIOrder() }
    }
