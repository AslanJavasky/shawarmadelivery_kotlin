...
import com.aslanjavasky.shawarmadelviry.data.repoImpls.redis.entity.*
...
class OrderRepoAdapter(
    private val orderRepository: OrderRedisRepository,
    private val userRepository: UserRedisRepository,
    private val menuItemRepository: MenuItemRedisRepository
    ...
    override fun getOrdersByUser(user: IUser): List<IOrder> {
        val ordersEntities = orderRepository.findByUserId(user.id!!.getUUIDFromLong())
        return ordersEntities.map { getIOrderById(it.id!!) }
    }
